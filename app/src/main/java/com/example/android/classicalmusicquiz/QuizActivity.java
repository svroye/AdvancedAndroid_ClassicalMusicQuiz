...
public class QuizActivity extends AppCompatActivity implements View.OnClickListener, ExoPlayer.EventListener {
    ...
    // DONE (1): Create a method that shows a MediaStyle notification with two actions
    ...
    private void showNotification(PlaybackStateCompat state){
        ...
        // the pending intent from the MediaButtonReceiver ensures that clicking the button calls
        // the appropriate MediaSession callback automatically
        NotificationCompat.Action actionPlayPause = new NotificationCompat.Action(
                icon, play_pause, MediaButtonReceiver.buildMediaButtonPendingIntent(this,
                PlaybackStateCompat.ACTION_PLAY_PAUSE));

        NotificationCompat.Action actionRestart = new android.support.v4.app.NotificationCompat.Action(
                R.drawable.exo_controls_previous, getString(R.string.exo_controls_previous_description),
                MediaButtonReceiver.buildMediaButtonPendingIntent(this,
                PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS));

        // pending intent for opening the activity when the notification is clicked
